...
public class TestCustomer {
	...
	public void testNewCustomer() {
		...
		Session session = factory.openSession();		//session can be openSession or getCurrentSession - multiple session in the connection pool
		Transaction txn = session.getTransaction();		
		
		try {
			txn.begin();
			session.save(c1);							// an object is passed which will parse the object 
														//	and does the required operation in the Database
			txn.commit();
		} catch (Exception e) {
			e.printStackTrace();
